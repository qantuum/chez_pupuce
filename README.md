...
*03-Aug-2018::Display Produits in Employes panel with Fournisseurs'names, using inner join SQL queries --
...
>>> Employe (user) has an interface to create an account, connect account session, disconnect account session, delete their account. The Employes can not update their account. The Employes get to see Clients, Produits and Fournisseurs lists.  
**TODO on this** --- the Employe must be able to also manage Fournisseur and Produits list, ideally I would add a "+", "pencil" and "trash" buttons on each list item. The Employes Update form has not been implemented (to keep time to go on different targets), but the update method is available in Employes class.  
