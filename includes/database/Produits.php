...
class Produits implements CRUD {

	// attributes
	protected $_id, $_nom, $_description, $_img_src, $_prix, $_stock ;
	...
	// THIS IS THE ONLY METHOD FROM PRODUITS, THAT IS ACTUALLY CALLED FROM APP
	public static function dataReadAll($database) { // stands for the "readInfo" on the class diagram
		...
		$array = get_database($database)->query(
			"SELECT `pupuce_produit_primary_id`, `pupuce_produit_description`, `pupuce_produit_image`, `pupuce_produit_prix`, `pupuce_produit_nom`, `pupuce_produit_stock`, `pupuce_fournisseur_nom`
			FROM `pupuce_produit`
	...
	public function dataNew(array $data) {
		$id;
		// ******************************************************to update with the correct table
